...
A system prototype with real GPU--CPU data movement on RTX 5080 confirms practical viability: the hierarchy matches full-cache accuracy while reducing HBM usage by 50\%, with only 5--7\% transfer overhead.
...
\begin{itemize}
    ...
    \item We conduct ablation studies across five importance scoring methods (including gradient-based), demonstrating that cumulative attention score is optimal for reasoning token importance estimation.
    \item We implement a system prototype with real GPU--CPU data movement, achieving 5--7\% transfer overhead and confirming that the hierarchy matches full-cache accuracy at 50\% HBM reduction.
\end{itemize}
...
We use two configurations: (1)~\emph{Algorithm experiments} (Sections~\ref{sec:hierarchy_vs_eviction}--\ref{sec:scoring_ablation}) use fp16 inference on NVIDIA RTX 6000 Ada (48\,GB, PCIe Gen4) to isolate the effect of tier assignment and scoring methods; (2)~\emph{System prototype experiments} (Section~\ref{sec:system}) use int8 quantization on NVIDIA RTX 5080 (16\,GB, PCIe Gen5 x16) to evaluate real GPU--CPU data movement under a practical deployment scenario.
Both use greedy decoding with a maximum of 2,048 new tokens.
The int8 configuration yields a higher full-cache baseline (76\% vs.\ 66\% on GSM8K) due to quantization-induced regularization effects observed in prior work; we therefore report \emph{relative accuracy retention} ($\text{accuracy}/\text{baseline}$) when comparing across configurations.

\paragraph{Datasets.} We evaluate on GSM8K~\cite{gsm8k} (grade-school math, 50 samples) and MATH-500~\cite{math} (competition mathematics, 50 samples).
...
Hierarchy configurations match or exceed full-cache accuracy while reducing HBM usage by 50--70\%.}
\label{tab:e2e}
\vskip 0.1in
\small
...
\midrule
Full (100\% GPU) & 76\% & 100\% & 21.8 & --- \\
\midrule
Eviction (50\% HBM) & 8\% & 11\% & 19.9 & --- \\
Eviction (30\% HBM) & 2\% & 3\% & 20.1 & --- \\
\midrule
Hierarchy (50\% HBM, 10\% evict) & \textbf{78\%} & \textbf{103\%} & 18.7 & 5.2\% \\
Hierarchy (30\% HBM, 10\% evict) & 72\% & 95\% & 18.7 & 5.4\% \\
Hierarchy (50\% HBM, 5\% evict) & 68\% & 89\% & 18.8 & 5.2\% \\
\bottomrule
\end{tabular}
\end{table}
...
Our system prototype demonstrates this concretely: on an RTX 5080 (16\,GB HBM), we reduce HBM usage by 50--70\% with only 14\% throughput loss and 5--7\% transfer overhead, while maintaining full-cache accuracy.
Extrapolating, a single NVIDIA RTX 4090 (24\,GB HBM) could serve reasoning chains that currently require A100 (80\,GB) capacity, by offloading 70--90\% of KV entries to the 64+\,GB of available DDR.
...
Algorithm experiments on GSM8K and MATH-500 demonstrate that our hierarchy preserves 91\% of baseline accuracy with only 3\% token eviction, while H2O-style eviction achieves only 2--30\% accuracy across all budget levels.
Our system prototype with real GPU--CPU data movement confirms these benefits translate to practice: the hierarchy matches full-cache accuracy (78\% vs.\ 76\%) while reducing HBM usage by 50\%, with only 5--7\% transfer overhead and 14\% throughput loss.
Ablation studies confirm that cumulative attention score is the optimal importance metric for reasoning tokens, outperforming VATP, redundancy-aware, and gradient-based alternatives.
