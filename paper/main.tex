...
On GSM8K with DeepSeek-R1-Distill-Qwen-7B, our approach retains 85\% of baseline accuracy (58\% vs.\ 68\%) while permanently evicting only 3\% of tokens.
...
With only 3\% eviction, accuracy reaches 58\% (85\% of the 68\% baseline), demonstrating that the overwhelming majority of tokens can be safely offloaded without information loss.

Our contributions are:
\begin{itemize}
    \item We characterize the long-tail importance distribution of reasoning tokens and demonstrate the cliff-effect failure mode of naive eviction strategies on reasoning LLMs.
...
At the same fraction of tokens permanently evicted, our memory hierarchy dramatically outperforms pure eviction.
With 10\% eviction, the hierarchy achieves 34\% accuracy compared to 0\% for pure eviction at equivalent memory savings.
With only 3\% eviction, accuracy reaches 58\%---85\% of the 68\% full-cache baseline.
...
The sweet spot lies at 3--5\% eviction, where 95--97\% of tokens are preserved (either in HBM or DDR) and accuracy remains within 12--15\% of the baseline.
...
% TODO: Fill in after running multi-benchmark experiments
\textit{Results on MATH-500 and AIME to be added.}
...
% TODO: Importance scoring comparison (attention vs gradient vs VATP)
\textit{Ablation on importance scoring methods to be added.}
...
Experiments on GSM8K demonstrate that our hierarchy preserves 85\% of baseline accuracy with only 3\% token eviction, while pure eviction at equivalent memory savings yields 0\%.
